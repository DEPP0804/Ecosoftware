...
class _AnualidadesPageState extends State<AnualidadesPage> {
            ...
            Card(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      'Fórmulas (Anualidad Ordinaria Vencida):',
                      style: textTheme.titleMedium?.copyWith(
                        color: colorScheme.primary,
                      ),
                    ),
  ...
  void _showCalculatorDialog(BuildContext context) {
    ...
    final TextEditingController futureValueController =
        TextEditingController(); // VF
    final TextEditingController presentValueController =
            ...
            void _calcularDialogo() {
              if (dialogFormKey.currentState?.validate() ?? false) {
                ...
                if ((selectedVariable == 'VF' ||
                        selectedVariable == 'VA' ||
                        selectedVariable == 'n' ||
                        selectedVariable == 'i_disabled') &&
                    a == null) {
                  _showErrorSnackBar(
                    dialogContext,
                    'Ingrese el Pago Periódico (A).',
                  );
                  return;
                }
                if ((selectedVariable == 'VF' ||
                ...
                if ((selectedVariable == 'VF' ||
                  ...
                  _showErrorSnackBar(
                    dialogContext,
                    'Ingrese el Número de Períodos (n).',
                  );
                  return;
                }
                ...
                if ((selectedVariable == 'A' ||
                  ...
                  _showErrorSnackBar(
                    dialogContext,
                    'Ingrese el Valor Futuro (VF) o el Valor Actual (VA).',
                  );
                  return;
                }
                if ((selectedVariable == 'A' ||
                ...
                try {
                  if (i <= 0 && selectedVariable != 'i_disabled')
                    throw Exception(
                      'Tasa debe ser positiva.',
                    ); // Validar tasa positiva (excepto al calcularla)
                  if (n != null && n <= 0 && selectedVariable != 'n')
                    throw Exception(
                      'Períodos debe ser positivo.',
                    ); // Validar periodos positivos

                  switch (selectedVariable) {
                    case 'VF':
                      if (a == null || n == null)
                        throw Exception('Faltan A o n');
                      if (i == 0) {
                        // Caso especial: interés cero
                        resultado = a * n;
                      } else {
                    ...
                    case 'n': // Calcular Número de Períodos
                      if (a == null || a <= 0)
                        throw Exception('Pago (A) debe ser positivo.');
                      if (i == 0) {
                        // Caso especial: interés cero
                        if (vf != null && a != 0)
                          resultado = vf / a;
                          ...
                          throw Exception(
                            'Se requiere VF o VA y A != 0 si i=0',
                          );
                      } else if (vf != null) {
                        // Calcular n basado en VF
                        if (vf <= 0) throw Exception('VF debe ser positivo.');
                        if ((vf * i / a) <= -1)
                          throw Exception(
                            'Valores inválidos para logaritmo (VF)',
                          );
                        resultado = log(1 + (vf * i / a)) / log(1 + i);
                      } else if (va != null) {
                        // Calcular n basado en VA
                        if (va <= 0) throw Exception('VA debe ser positivo.');
                        if ((va * i / a) >= 1)
                          throw Exception(
                            'Valores inválidos para logaritmo (VA)',
                          );
                    ...
                    case 'i_disabled': // Tasa de interés deshabilitada
                      throw Exception(
                        'El cálculo de la tasa (i) requiere métodos numéricos y no está implementado.',
                      );
                    default:
                      throw Exception('Variable desconocida');
                  }
                    ...
                    children: [
                      DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                            value: 'VF',
                            child: Text('Calcular Valor Futuro (VF)'),
                          ),
                          DropdownMenuItem(
                            value: 'VA',
                            child: Text('Calcular Valor Actual (VA)'),
                          ),
                          DropdownMenuItem(
                            value: 'A',
                            child: Text('Calcular Pago Periódico (A)'),
                          ),
                          DropdownMenuItem(
                            value: 'n',
                            child: Text('Calcular Núm. Períodos (n)'),
                          ),
                        ],
                      ...
                      if (selectedVariable != 'A')
                        _buildDialogTextField(
                          ...
                          label: 'Pago Periódico (A)',
                        ),
                      if (selectedVariable !=
                          'i_disabled') // Siempre mostrar tasa, excepto si se calculara
                        _buildDialogTextField(
                          controller: rateController,
                          label: 'Tasa Interés por Período (%)',
                        ),
                      ...
                      if (selectedVariable == 'A' ||
                        ...
                        _buildDialogTextField(
                          ...
                          label: 'Valor Futuro (VF) (Opcional)',
                        ),
                      if (selectedVariable == 'A' ||
                          selectedVariable == 'n' ||
                          selectedVariable == 'i_disabled')
                        _buildDialogTextField(
                          ...
                          label: 'Valor Actual (VA) (Opcional)',
                        ),
        ...
        validator: (value) {
          if (!readOnly) {
            ...
            if (!label.contains('Tasa') &&
                ...
                !label.contains('Valor')) {
              // Permitir VA/VF 0 si es input, pero no A o n
              if (!label.contains('Pago') && !label.contains('Períodos'))
                return null;
              return 'Positivo';
            }
          }
