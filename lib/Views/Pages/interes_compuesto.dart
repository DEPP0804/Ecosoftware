...
class _InteresCompuestoPageState extends State<InteresCompuestoPage> {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildFormulaDetailText(
                      'A (ó VF) = Monto futuro o valor final',
                      textTheme,
                    ),
                    _buildFormulaDetailText(
                      'P (ó VP) = Principal o valor presente',
                      textTheme,
                    ),
                    _buildFormulaDetailText(
                      'i (ó r) = Tasa de interés por período',
                      textTheme,
                    ),
                    _buildFormulaDetailText(
  ...
  void _showCalculatorDialog(BuildContext context) {
    ...
    final TextEditingController amountController =
        TextEditingController(); // Monto Futuro (A)
            ...
            void _calcularDialogo() {
              if (dialogFormKey.currentState?.validate() ?? false) {
                ...
                double? n = double.tryParse(
                  timeYearsController.text,
                ); // Asumiendo n en años y tasa anual
                ...
                if ((selectedVariable == 'A' ||
                        ...
                        selectedVariable == 'r') &&
                    n == null) {
                  _showErrorSnackBar(dialogContext, 'Ingrese el Tiempo (n).');
                  return;
                }
                  ...
                  switch (selectedVariable) {
                    case 'A': // Monto Total
                      if (p == null || n == null)
                        throw Exception(
                          "Valores inválidos",
                        ); // Redundante por validación previa
                    ...
                    case 'P': // Principal
                      ...
                      if (rDecimal <= -1)
                        throw Exception("Tasa inválida para calcular P");
                      resultado = a / pow((1 + rDecimal), n);
                      break;
                    case 'r': // Tasa de Interés
                      if (a == null || p == null || n == null)
                        throw Exception("Valores inválidos");
                      if (a <= 0 || p <= 0)
                        throw Exception(
                          "A y P deben ser positivos para calcular r.",
                        );
                      if (n == 0)
                        throw Exception(
                          "Tiempo no puede ser cero para calcular r.",
                        );
                      // r = (A/P)^(1/n) - 1
                      resultado =
                          (pow((a / p), (1 / n)) - 1) * 100; // Mostrar como %
                      break;
                    case 'n': // Tiempo
                      if (a == null || p == null)
                        throw Exception("Valores inválidos");
                      if (a <= 0 || p <= 0)
                        throw Exception(
                          "A y P deben ser positivos para calcular n.",
                        );
                      if (rDecimal <= 0)
                        throw Exception(
                          "Tasa debe ser positiva para calcular n.",
                        );
                      // n = log(A/P) / log(1+r)
                      if ((a / p) <= 0 || (1 + rDecimal) <= 0)
                        throw Exception("Valores inválidos para logaritmo.");
                    ...
                    children: [
                      DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                            value: 'A',
                            child: Text('Calcular Monto Total (A)'),
                          ),
                          DropdownMenuItem(
                            value: 'P',
                            child: Text('Calcular Principal (P)'),
                          ),
                          DropdownMenuItem(
                            value: 'r',
      ...
      child: TextFormField(
        ...
        validator: (value) {
          if (!readOnly) {
            ...
            if (!label.contains('Tasa') && double.parse(value) <= 0) {
              return 'Positivo';
            }
          }
          return null;
