...
class _LoginPageState extends State<LoginPage>
  ...
  String _mapFirebaseAuthExceptionMessage(String? code) {
    switch (code) {
      case 'user-not-found':
        return 'Correo electrónico no encontrado.'; // Email/Pass login
      case 'wrong-password':
  ...
  Future<void> _performAuthAction(Future<void> Function() action) async {
    ...
    try {
      print("[PerformAuth] Awaiting action()...");
      await action();
      print("[PerformAuth] action() completed successfully.");
    } on FirebaseAuthException catch (e) {
      print("[PerformAuth] FirebaseAuthException caught: ${e.code}");
      if (mounted)
        setState(() {
          _errorMessage = _mapFirebaseAuthExceptionMessage(e.code);
        });
    } catch (e) {
      print("[PerformAuth] Generic Exception caught: ${e.toString()}");
      if (mounted)
        setState(() {
          _errorMessage =
              'Ocurrió un error: ${e.toString().replaceFirst("Exception: ", "")}';
        });
    } finally {
      print("[PerformAuth] Finally block reached. Setting isLoading = false.");
      if (mounted) {
  ...
  Future<void> _resetPassword() async {
    if (_emailController.text.trim().isEmpty ||
      ...
      setState(
        () => _errorMessage = 'Ingresa tu correo asociado para restablecer.',
      );
      return;
    }
    if (_isLoading) return;
  ...
  Widget _entryField(
    ...
    return TextFormField(
      ...
      decoration: InputDecoration(
        labelText: title + (isOptional ? " (Opcional)" : ""),
        prefixIcon:
            prefixIcon != null
      ...
      validator:
          ...
          (value) {
            ...
            if (title.contains('Correo') && !value.contains('@')) {
            ...
            Align(
              ...
              child: Container(
                height:
                    ...
                    (_isLogin ? 0.65 : 0.90), // Reducida altura para Login
                      ...
                      children: [
                        ...
                        if (!_isLogin) ...[
                          ...
                          _entryField(
                            ...
                            validator: (v) {
                              if (v == null || v.isEmpty) return 'Confirma';
                        ...
                        if (_isLogin) ...[
                          ...
                          Align(
                            ...
                            child: _forgotPasswordButton(theme),
                          ),
                          const SizedBox(
                            height: kDefaultPadding * 1.5,
                          ), // Más espacio antes del error
                        ],
